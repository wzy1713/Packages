# -*- coding: utf-8 -*-
"""
Created on Mon May 14 09:44:16 2018
...
def conv_net(_X, _weights, _biases, _dropout):
    ...
    conv1 = max_pool(img = conv1, size = 3, k=2)
    # Apply Dropout
    ...
    # Max Pooling (down-sampling)
    conv2 = max_pool(conv2, size = 3, k=2)
    # Apply Dropout
...
#loss = tf.reduce_mean(-tf.reduce_sum(y * tf.log(pred_), reduction_indices=[1]))
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        ...
        if step % display_step == 0:
            ...
            loss_ = sess.run(loss, feed_dict={x: mnist.test.images, 
                                              y: mnist.test.labels, 
                                              keep_prob: 1.})
            print("Iter " + str(step*batch_size) + ", Minibatch Loss= " + "{:.6f}".format(loss_) + ", Training Accuracy= " + "{:.5f}".format(acc))
            # AUC
            #auc = sess.run(auc, feed_dict = {x: batch_xs, y: batch_ys, keep_prob: 1.})
        step += 1
